...
class ExampleKafkaResourceIT {
    ...
    void setup() {
        ...
        ExampleKafkaResource kafkaResource = new ExampleKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/example-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/example-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
